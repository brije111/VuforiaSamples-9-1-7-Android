...
class RefFreeFrameGL
{
    ...
    private final class TEXTURE_NAME
    {
        ...
        final static int TEXTURE_VIEWFINDER_MARKS_PORTRAIT = 0;
        
        // Viewfinder in landscape mode
        final static int TEXTURE_VIEWFINDER_MARKS = 1;
        
        // Not a texture, count of predef textures
        final static int TEXTURE_COUNT = 2;
    }
    ...
    private Matrix44F projectionOrtho, modelview;
    ...
    private final int NUM_FRAME_VERTEX_TOTAL = 4;
    private final int NUM_FRAME_INDEX = 1 + NUM_FRAME_VERTEX_TOTAL;

    private final float[] frameVertices_viewfinder = new float[NUM_FRAME_VERTEX_TOTAL * 3];
    private final float[] frameTexCoords = new float[NUM_FRAME_VERTEX_TOTAL * 2];
    private final short[] frameIndices = new short[NUM_FRAME_INDEX];
    
    // Portrait/Landscape status detected in init()
    private boolean isActivityPortrait;

    private static final String FRAME_VERTEX_SHADER = " \n" + "attribute vec4 vertexPosition; \n"
    ...
    RefFreeFrameGL(UserDefinedTargets activity)
    {
        mActivity = activity;
        shaderProgramID = 0;
        vertexHandle = 0;
    ...
    void setModelViewScale()
    {
        if (modelview != null)
        {
            float[] tempModelViewData = modelview.getData();
            tempModelViewData[14] = MODEL_VIEW_SCALE;
    ...
    boolean init(int screenWidth, int screenHeight)
    {
        ...
        if ((shaderProgramID = SampleUtils.createProgramFromShaderSrc(
                FRAME_VERTEX_SHADER, FRAME_FRAGMENT_SHADER)) == 0)
            return false;
    ...
    {
        if (textures == null || modelview == null)
            return;
