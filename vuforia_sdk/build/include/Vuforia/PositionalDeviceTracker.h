 ...
 * will attempt to initialize automatically, but this requires some motion of
 * the device. If the device is stationary, it will wait for sufficient motion
 * before initialization is completed. You can use TrackableResult::getStatus()
 * and TrackableResult::getStatusInfo() on the DeviceTrackableResult to determine 
 * whether the PositionalDeviceTracker is still initializing.
 *
...
public:
     ...
     * yourself. A call to reset() invalidates (destroys) all Anchors. Do not access
     * the returned pointer after calling reset().
     *
