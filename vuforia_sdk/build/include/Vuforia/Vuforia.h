...
enum INIT_ERRORCODE {
    ...
    INIT_LICENSE_ERROR_BAD_REQUEST = -12            ///< Unable to verify license key due to bad request
};
...
enum PIXEL_FORMAT {
    ...
    NV21 = 32,                  ///< A color pixel stored in 12 or more bits
                                ///< using Y, U and V planes
    NV12 = 64,                  ///< A color pixel stored in 12 or more bits
                                ///< using Y, U and V planes
    YV12 = 128,                 ///< A color pixel stored in 12 or more bits
                                ///< using discreet Y, U and V planes
    YUV420P = 256,              ///< A color pixel stored in 12 or more bits
                                ///< using discreet Y, U and V planes
    YUYV = 512,                 ///< A color pixel stored in 12 or more bits
                                ///< using interleaved Y, U and V planes
};
    
/// Model Target tracking modes to optimize tracking quality and robustness for different target types.
/// Use when calling setHint(HINT_MODEL_TARGET_MODE, static_cast<int>(MODEL_TARGET_MODE))
enum MODEL_TARGET_MODE
{
    MODEL_TARGET_MODE_DEFAULT,        ///< Model Target tracking parameters optimized to work well on a broad range of object types.
    MODEL_TARGET_MODE_CARS            ///< Model Target tracking parameters optimized for car exteriors.
};
...
enum HINT {
    ...
    /**
     *  This hint tells the tracker how many images shall be processed
    ...
    */
     ...
     * It needs to be set after the ObjectTracker has been initialized.
     * Valid values for the hint are 0 or 1.
     * When set to 0, the recognition engine is stopped for Advanced (360) datasets after a model
     * target has been found and is never turned on again automatically. The dataset needs to be
     ...
     * If the recognition engine finds a new target in the image frame, tracking will be
     * switched to the newly identified target, resulting in tracking loss of the prior model target
     * Default is: 1
     **/
    ...
    /**
     * This hint modifies the internal Model Target tracking parameters to optimize
     * the tracking quality and robustness for different target types.
     *
     * \note The hint can only be set when the ObjectTracker is stopped.
     *
     * Valid values for the hint can be found in MODEL_TARGET_MODE.
     * \note to use this flag with setHint() method you need to static cast the value to an int
     * Default is: MODEL_TARGET_MODE::MODEL_TARGET_MODE_DEFAULT
     */
    HINT_MODEL_TARGET_MODE = 5,
};
 ...
 *
 * If you restrict Fusion to FUSION_PROVIDER_VUFORIA_VISION_ONLY, Ground Plane and AreaTarget
 * will cease to work and ModelTargets will operate in a less robust manner.
 *
 * If you restrict Fusion to FUSION_PROVIDER_VUFORIA_SENSOR_FUSION, AreaTarget
 * will cease to work.
 * 
 ...
 *
 * This API call should be made after Vuforia::init(). For instance to use
...
/// Get the version of the %Vuforia SDK as a C-style string. The format is three numeric
/// components, separated by dot.  The components are major version, minor version and
/// change version. The memory returned by this function is static and does not need to be
/// freed.
VUFORIA_API const char* getLibraryVersion();

/// Get the build ID of the %Vuforia SDK as a C-style string. The format is a numeric
/// string representation of the build ID. The memory returned by this function is static
 ...
 * functionality during subsequent calls to Vuforia::init().
